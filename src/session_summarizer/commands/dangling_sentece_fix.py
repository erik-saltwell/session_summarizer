...
from ..helpers.dangling_sentence_fixer import DanglingSentenceFixResult, fix_dangling_sentences
...
class DanglingSentenceFixCommand(SessionProcessingCommand):
    ...
    def process_session(self, settings: SessionSettings, session_dir: Path) -> None:
        ...
        fix_result: DanglingSentenceFixResult = fix_dangling_sentences(
            settings, session_dir, input_clips, self, self.logger
        )
        self.logger.report_message(f"Fixed {fix_result.fixed_count} dangling sentence fragment(s).")
        self.save_speech_clip(fix_result.clips, session_dir, settings.paths.dangling_sentence_fix)
